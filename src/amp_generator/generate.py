...
def download_large_weights():
    import os
    from pathlib import Path
    try:
        import gdown
    except ImportError:
        raise ImportError("❌ 找不到 gdown 库！请在终端运行 `uv pip install gdown` 或 `pip install gdown`")

    # 获取项目的绝对根目录（当前文件 generate.py 的上三级目录，确保路径万无一失）
    ROOT_DIR = Path(__file__).resolve().parent.parent.parent
    
    # 替换为真实的 Google Drive File ID
    files_to_download = {
        ROOT_DIR / "checkpoints/diffusion_amp_diffusion_50len_150ep/best.pth": "1LMFWP2prFGRsgHQFHZ5gQMhGiu9ps50C",
        ROOT_DIR / "amp_feature_comparison_results/models/amp_classifier_aac_dde.joblib": "18YVSQzb30FGMHed1rRJGydhf301-yTBI",
        ROOT_DIR / "amp_feature_comparison_results/models/hemolysis_classifier.joblib": "185AXy2Qyy_mYhKlyN35NcDkSQd3s9xl1",
        ROOT_DIR / "checkpoints/esm2/model.safetensors": "10qgOIS_Kz4jAL8uuEMyqUewfCN8Qaktq"
    }
    
    for file_path, file_id in files_to_download.items():
        # 本地缓存强校验：如果文件存在且大于 10MB，则认为已下载，直接跳过 (防重复下载报错)
        if file_path.exists() and file_path.stat().st_size > 10 * 1024 * 1024:
            print(f"[提示] 检测到有效本地缓存，跳过下载: {file_path.name}")
            continue
            
        print(f"\n[提示] 准备从 Google Drive 下载 {file_path.name} ...")
        file_path.parent.mkdir(parents=True, exist_ok=True)
        
        # 构建 Google Drive 专用下载直链
        url = f'https://drive.google.com/uc?id={file_id}'
        
        try:
            # gdown 内部自动处理了 Cookies，完美绕过大文件病毒扫描警告，且自带极佳的进度条显示
            gdown.download(url, str(file_path), quiet=False)
        except Exception as e:
            print(f"下载异常: {e}")
            
        # 二次安全校验 (防止下到了几KB的网页报错文件)
        if not file_path.exists() or file_path.stat().st_size < 1024 * 1024:
            raise Exception(f"❌ {file_path.name} 下载失败或文件过小！请检查 File ID 及云盘权限设置。")
...
def main():
    ...
    parser.add_argument('--classifier', type=str, default='amp_feature_comparison_results/models/amp_classifier_aac_dde.joblib', help='抗菌活性打分器路径')
    parser.add_argument('--hemo-classifier', type=str, default='amp_feature_comparison_results/models/hemolysis_classifier.joblib', help='溶血毒性打分器路径')
    ...
    MODEL_TOKEN_LEN = 51  # 强制固定为训练时的长度，防止官方传 --length 40 测试时崩溃                 
